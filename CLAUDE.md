...
| 问题 | 描述 | 优先级 | 状态 |
|------|------|--------|------|
| 排序字段 SQL 注入 | `RequestParse` 中排序字段直接拼接，未验证是否属于视图字段 | P0 | 已修复 |
| 视图名称验证 | `viewName` 路径参数应验证是否只包含安全字符 | P0 | 已修复 |

### 代码规范

| 问题 | 描述 | 优先级 | 状态 |
|------|------|--------|------|
| 包名拼写 | `contoller` 包应重命名为 `controller` | P2 | 已修复 |
| 未使用常量 | `QueryUtils.QUERY_PARAMETER_REGEXP` 未使用，应删除 | P3 | 已修复 |
| 异常类型 | `FilterOp.fromStringOp()` 应抛出 `QueryBeanSqlException` 而非 `IllegalArgumentException` | P3 | 已修复 |
